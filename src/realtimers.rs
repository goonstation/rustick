use meowtonin::{byond_fn, ByondValue,ByondResult,ByondError};
...
#[byond_fn]
pub fn schedule_once(delay: u64, owning_obj: ByondValue, proc_path: ByondValue, proc_args: ByondValue) -> ByondResult<String> {
    let id = Uuid::new_v4();
    ...
    if can_have_procs(&owning_obj) { // Meowtonin catches panics and converts them to runtimes, but if the closure here panics the timing thread dies and you won't find out (subsequent calls might panic in the meowtonin thread to let you know tho)
        owning_obj.inc_ref();
...
#[byond_fn]
pub fn schedule_periodic(delay: u64, period: u64, owning_obj: ByondValue, proc_path: ByondValue, proc_args: ByondValue) -> ByondResult<String> {
    let id = Uuid::new_v4();
    let delay = Duration::from_millis(delay);
    ...
    if can_have_procs(&owning_obj) { // Meowtonin catches panics and converts them to runtimes, but if the closure here panics the timing thread dies and you won't find out (subsequent calls might panic in the meowtonin thread to let you know tho)
        timers.schedule_action_periodic(id, delay, period, move |_timer_id|{ 
            match call_owned_proc(&owning_obj, &proc_path, &proc_args) {
                Ok(ret) => {
        ...
        timers.schedule_action_periodic(id, delay, period, move |_timer_id|{ 
            match call_global_proc(&proc_path, &proc_args) {
                Ok(ret) => {
                    let res = should_reschedule(ret);
                    if res == TimerReturn::Cancel {
                        proc_args.dec_ref(); 
...
pub fn call_global_proc(proc_path_bv: &ByondValue, proc_args_bv: &ByondValue) -> ByondResult<Option<String>>
{
...
pub fn call_owned_proc(proc_owner: &ByondValue, proc_path_bv: &ByondValue, proc_args_bv: &ByondValue) -> ByondResult<Option<String>>
{
    let proc_path = proc_path_bv.get_string()?;
    let proc_args = proc_args_bv.read_list()?;
